...
bool Configuracion::exportarDatos()
{
    ...
    for (const string& nombre : archivos)
    {
        ...
        if (origen == nullptr)
        {
            cout << "No se encontró el archivo: " << nombre << endl;
            ok = false;
            continue;
        }
...
bool Configuracion::generarCSV(const string& nombreArchivoBinario, const string& nombreCSV)
{
    ...
    else if (nombreArchivoBinario == "clientes.dat") tipoArchivo = 2;
    else if (nombreArchivoBinario == "usuarios.dat") tipoArchivo = 3;
